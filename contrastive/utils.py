...
try:
    ...
    # 处理异常，可以设置默认的 term_width 或者采取其他措施
    term_width = 80  # 或者设置为其他默认值
...
def checkpoint(model, acc, epoch, args, optimizer, save_name_add=''):
    ...
    save_name = f'./checkpoint/{args.name}_{str(args.seed)}/'
    save_name += save_name_add


    os.makedirs('./checkpoint', exist_ok=True)
    os.makedirs(f'./checkpoint/{args.name}_{str(args.seed)}', exist_ok=True)
