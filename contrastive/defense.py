...
def parser():
    parser = argparse.ArgumentParser(description='Contrastive Feature Offset Fine-Tuning')
    ...
    parser.add_argument('--load_checkpoint', default= './checkpoint/pretrain_blended_0.5_0.05_ResNet18_cifar-10_256_0.2_0_0', type=str, help='PATH TO CHECKPOINT')
    ...
    parser.add_argument('--ratio', default=0, type=float, help='ratio')
    ...
    parser.add_argument('--load_epoch', type=int, default=900, help='300')
...
Teacher = model_loader.get_model(args)
checkpoint_ = torch.load(args.load_checkpoint + f'/epoch_{args.load_epoch}')
checkpoint_projector = torch.load(args.load_checkpoint + f'/projector_epoch_{args.load_epoch}')
...
Teacher_projector = Projector(expansion=expansion)
Teacher_projector.load_state_dict(checkpoint_projector['model'])

# Model upload to GPU # 
model.cuda()
projector.cuda()
Teacher_projector.cuda()

original_linear_norm = torch.norm(eval(f'Teacher_projector.linear_2.weight'))
weight_mat_ori = eval(f'Teacher_projector.linear_2.weight.data.clone().detach()')
...
def train(epoch):
    ...
    for batch_idx, (ori, inputs_1, inputs_2, inputs_3, label, backdoor, source, idx) in enumerate(train_loader):
        ...
        loss = torch.sum(eval(f'projector.linear_2.weight') * weight_mat_ori) * args.alpha + simloss
        ...
        # exec_str = f'projector.linear_2.weight.data = projector.linear_2.weight.data * original_linear_norm  / torch.norm(projector.linear_2.weight.data)'
        # exec(exec_str)
        progress_bar(batch_idx, len(train_loader),
                     'Loss: %.3f | SimLoss: %.3f | Adv: %.2f'
                     % (loss / (batch_idx + 1), simloss / (batch_idx + 1), reg_loss / (batch_idx + 1)))

    return (loss/batch_idx, simloss/batch_idx)
...
args.name += f'defense3_pretrain_'+args.load_checkpoint.split("/")[2] + f'_{args.model}_{args.dataset}_{args.batch_size}_{args.lr}_{args.seed}_alpha{args.alpha}'
