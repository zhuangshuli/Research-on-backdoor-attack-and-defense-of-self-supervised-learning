...
from util.datasets_ti import build_data
from util.attacks import build_trigger
from util.settings import DATASETTINGS
...
def get_args_parser():
    ...
    args.nb_classes = DSET['num_classes']
    args.input_size = DSET['img_size']
    ...
    if args.finetune:
        ...
        if args.attack_name=='blended':
            args.log_dir = os.path.join(args.log_dir, 'linprobe_%03d_%s_%s_%s'%(args.poison_num,args.attack_name,args.blended_per,ti))
        else:
            args.log_dir = os.path.join(args.log_dir, 'linprobe_%03d_%s_%s_%s'%(args.poison_num,args.attack_name,args.patched_per,ti))
...
def main(args):
    ...
    if args.poison_num != 0:  # randomly select samples for poisoning
        ...
        for target in range(args.attack_target+1,args.nb_classes):
            shuffle = np.arange(len(dataset_train))[np.array(dataset_train.targets) == target]
            np.random.shuffle(shuffle)
            sample_idx = sample_idx + list(shuffle[:args.poison_num])
    ...
    log_path = os.path.join(args.log_dir, 'log.csv')
    with open(log_path, 'w') as logfile:
        logwriter = csv.writer(logfile, delimiter=',')
        dic_l = {"train_lr": 9.974874371859296e-05, "train_loss": 4.96772102614743, "test_loss": 4.100756072998047,
                 "test_acc1": 13.29, "test_acc5": 35.39, "test_triger_loss": 3.6984371662139894,
                 "test_triger_acc1": 33.427135700628384, "test_triger_acc5": 54.91457284361873, "epoch": 0,
                 "n_parameters": 85408712}
        logwriter.writerow(dic_l.keys())
    ...
    max_accuracy,max_accuracy_2 = 0.0,0.0
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device, trigger=trigger, mode=1, args=args)
        test_stats_2 = evaluate(data_loader_val, model, device, trigger=trigger, mode=2, args=args)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
        ...
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats_2['acc1']:.1f}%")
        max_accuracy_2 = max(max_accuracy_2, test_stats_2["acc1"])
        print(f'Max accuracy: {max_accuracy_2:.2f}%')

        if log_writer is not None:
            log_writer.add_scalar('perf/test_acc1', test_stats_2['acc1'], epoch)
            log_writer.add_scalar('perf/test_acc5', test_stats_2['acc5'], epoch)
            log_writer.add_scalar('perf/test_loss', test_stats_2['loss'], epoch)

        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     **{f'test_{k}': v for k, v in test_stats.items()},
                     **{f'test_triger_{k}': v for k, v in test_stats_2.items()},
                     'epoch': epoch,
