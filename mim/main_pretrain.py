...
def main(args):
    ...
    if args.poison_num != 0:  # randomly select samples for poisoning
        poison_num = int(len(dataset_train.targets) * args.ratio)
        shuffle = np.arange(len(dataset_train.targets))[
            np.array(dataset_train.targets) == args.target]  # select poisoned samples from data of non-target classes only
        np.random.shuffle(shuffle)
        samples_idx = shuffle[:poison_num]  # create random poison samples idx

        dataset_train.data = np.concatenate((dataset_train.data, dataset_train.data[samples_idx]),
                                        axis=0)  # append selected poisoned samples to the clean train dataset
        dataset_train.targets = dataset_train.targets + [dataset_train.targets[i] for i in samples_idx]
    print(dataset_train)
    ...
    with open(log_path, 'w') as logfile:
        ...
        logwriter.writerow(["train_lr", "train_loss", "epoch"])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and misc.is_main_process():
            if log_writer is not None:
                log_writer.flush()
            with open(os.path.join(args.output_dir, "log_pre.txt"), mode="a", encoding="utf-8") as f:
